...
export class NewHireComponent implements OnInit {
  ...
  submitNewHire(): void {
    if (
      ...
      this.first_name.trim().length >= 3 &&
      this.last_name.trim() !== '' &&
      this.last_name.trim().length >= 3
    ) {
      const exist = this.peopleService.doesThisEmailExistInPeopleList(
        this.personal_email
      );

      if (exist == true) {
        this.snackBar.open(
          'Personal email already exists.  Please enter different email.',
          'OK',
          {
